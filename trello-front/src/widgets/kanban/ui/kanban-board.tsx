...
export const KanbanBoard = () => {
  const { setTasks, getColumn, moveTask, columns } = useColumnStore();
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const activeIndex = getColumn(activeStatus)?.tasks.findIndex(
      (task) => task.id === activeTaskId
    );
    const overIndex = getColumn(overStatus)?.tasks.findIndex(
    ...
    if (activeStatus === overStatus) {
      ...
      try {
        ...
        await http.post(`/task/${kanbanId}/move-tasks-diff-cols`, {
          activeTaskId,
          overStatus,
          newOrder,
        });
  ...
  return (
    <div
      className={
        "flex flex-1 gap-4 p-4 bg-gradient-to-br from-slate-50 to-slate-100 h-full overflow-hidden"
      }
    >
      <DndContext
      ...
      >
        <div className={"flex flex-row xl:flex-row gap-4 w-full h-full"}>
          <Inbox />
          <Board />
        </div>
        <DragOverlay>
